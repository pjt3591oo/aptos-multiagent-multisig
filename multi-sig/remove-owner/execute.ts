...
const owner4 = Account.fromPrivateKey({
  privateKey: new Ed25519PrivateKey('0x3b02b54c8afea972f7c74bbfd911e1ef7767964d73d56189e42983d5f36e6742')
});
...
const executeMultiSigRemoveOwner = async () => {
  ...
  const removeOwnerReponse = await aptos.transaction.submit.simple({
    senderAuthenticator: owner2Authenticator,
    transaction,
  });
  await aptos.waitForTransaction({ transactionHash: removeOwnerReponse.hash });
};

async function main() {
  await executeMultiSigRemoveOwner()
}
